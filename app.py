...
def init_db() -> None:
    with sqlite3.connect(DB_PATH) as conn:
        ...
        conn.execute(
            """
            INSERT OR IGNORE INTO users (employee_id, employee_name, role)
            VALUES ('ADMIN', '系統管理員', 'admin')
            """
        )
...
def upsert_user(employee_id: str, employee_name: str, role: str = "employee") -> None:
    with get_db_connection() as conn:
        conn.execute(
            """
            INSERT INTO users (employee_id, employee_name, role) VALUES (?, ?, ?)
...
def insert_record(employee_id: str, employee_name: str, event_type: str) -> None:
    ...
    upsert_user(employee_id, employee_name, "employee")
    with get_db_connection() as conn:
        conn.execute(
            """
...
def update_attendance_record(record_id: int, employee_id: str, employee_name: str, event_type: str, created_at: str) -> tuple[dict, dict] | None:
    with get_db_connection() as conn:
...
def insert_audit_log(admin_employee_id: str, action: str, target_record_id: int, old_value: dict, new_value: dict) -> None:
    with get_db_connection() as conn:
        conn.execute(
            """
...
.card {{ max-width: 520px; margin: 50px auto; background:#fff; padding:20px; border-radius:12px; box-shadow:0 4px 14px rgba(0,0,0,.08); }}
...
<body><div class=\"card\"><h1>登入</h1>
<p>管理員帳號固定為 <b>ADMIN</b>，預設密碼 <b>admin123</b>（請上線後自行更改）。</p>
...
<input type=\"text\" name=\"employee_name\" placeholder=\"員工姓名（員工登入用）\" />
<input type=\"password\" name=\"password\" placeholder=\"管理員密碼（管理員登入必填）\" />
<button type=\"submit\">登入</button>
    ...
    def _handle_login(self, form_data: dict[str, list[str]]) -> None:
        ...
        if role == "admin":
            ...
            if employee_id != "ADMIN" or hashed != ADMIN_PASSWORD_HASH:
                self._redirect(f"/login?msg={urllib.parse.quote('管理員帳密錯誤')}")
                return
            employee_name = "系統管理員"
            upsert_user("ADMIN", employee_name, "admin")
        else:
            upsert_user(employee_id, employee_name, "employee")
...
<p>登入者：{html.escape(session['employee_id'])} / {html.escape(session['employee_name'])} ｜ <a href=\"/logout\">登出</a></p>
...
def run() -> None:
    ...
    try:
        ...
        print("啟動失敗：找不到可用的連接埠，請關閉占用程式後重試，或設定 PORT 環境變數。")
        raise
